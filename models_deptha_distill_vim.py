...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        x = self.patch_embed(x) # [12, 196, 192]
        ...
        # mamba impl
        residual = None
        hidden_states = x # torch.Size([bs, 197, 192])
        if not self.if_bidirectional:
            for idx, layer in enumerate(self.layers):
                ...
                ) # hidden_states torch.Size([bs, 197, 192]) residual torch.Size([bs, 197, 192])
    ...
    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        x = self.forward_features(x, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank) # [bs, 192]
        if return_features:
            return x
        x = self.head(x)
...
class DPT_Vim(nn.Module):
    def __init__(self, encoder='vitl', features=256, out_channels=[256, 512, 1024, 1024], embed_dim=192, patch_size=14,
                 use_bn=False, use_clstoken=False, localhub=True, aligned_blks_indices=[-4, -3, -2, -1], 
                 embedding_distillation_func="L1", aligned_feature_projection_mode='mlp-1layer', 
                 aligned_feature_projection_dim=[192, 384]):
        super(DPT_Vim, self).__init__()
        
        self.encoder = VisionMamba(patch_size=14, stride=14, embed_dim=192, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=False, if_devide_out=True, use_middle_cls_token=True)
        ...
        if aligned_feature_projection_mode is not None:
            ...
            aligned_feature_projection_dim = [embed_dim, dim]
    ...
    def forward_encoder(self, x):
        h, w = x.shape[-2:]
        # torch.Size([12, 784, 128]), torch.Size([12, 196, 160]), torch.Size([12, 49, 320]), torch.Size([12, 49, 320])
    ...
    def forward_decoder(self, features, h, w):
        ...
        features_reshape = [projection_head(feature) for feature, projection_head in zip(features, self.aligned_feature_projection_heads)]

        # tranform features to torch tensor
        features_reshape = torch.stack(features_reshape, dim=0)

        depth = self.depth_head(features_reshape, patch_h, patch_w)
    ...
    def forward_distillation_loss_embedding(self, features_teacher, features_student):
        '''
        features_teacher: list of tensor [torch.Size([12, 256, 1024]), torch.Size([12, 256, 1024]), torch.Size([12, 256, 1024]), torch.Size([12, 256, 1024])]
        features_student: list of tensor [torch.Size([12, 784, 128]), torch.Size([12, 196, 160]), torch.Size([12, 49, 320]), torch.Size([12, 49, 320])]
        '''
        ...
        # if type(features_teacher) == tuple:
        #     features_teacher = [feature[0] for feature in features_teacher]
    ...
    def infer(self, x):
        ...
        print("encoder_time", elapsed_time)
        pred = self.forward_decoder(features_student, h, w)
...
class DPT_DINOv2(nn.Module):
    ...
    def forward_encoder(self, x):
        ...
        features = self.pretrained.get_intermediate_layers(x, 4, return_class_token=False) # [visual_token, class_token]
