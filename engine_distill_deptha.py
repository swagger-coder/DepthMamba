...
def train_one_epoch(model: torch.nn.Module, model_teacher: torch.nn.Module,
    ...
    for data_iter_step, (samples, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            with torch.no_grad():
                features_teacher, h, w = \
                    model_teacher.module.forward_encoder(imgs)
                teacher_prediction = model_teacher.module.forward_decoder(features_teacher, h, w)  
            loss, loss_distillation_embedding, _ = model(imgs, features_teacher, teacher_prediction)
