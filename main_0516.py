...
from models_0516 import DPT_Vim, DepthAnything

from engine_0516 import train_one_epoch
...
def get_args_parser():
    ...
    parser.add_argument('--teacher_type', default='vits', type=str, help='Type of teacher model')
...
def main(args):
    ...
    model_teacher = DepthAnything.from_pretrained('LiheYoung/depth_anything_{}14'.format(args.teacher_type)).to(device).eval()

    if True:
        model.depth_head.load_state_dict(model_teacher.depth_head.state_dict())
        model.depth_head.requires_grad_(False)
    ...
    optimizer = optim_factory.create_optimizer_v2(model_or_params = model_without_ddp, opt = 'adamw', lr=args.lr, weight_decay=args.weight_decay, betas=(0.9, 0.95))
